...
@section('content')
    <div class="container">
        ...
        {{-- DEBUG --}}
        <p>ID: {{ $publisher->id }} | Nome: {{ $publisher->name }} | Endereço atual: "{{ $publisher->address ?? 'N/A' }}"
        </p>

        <form action="{{ route('publishers.update', $publisher) }}" method="POST" onsubmit="console.log('SUBMIT OK!')"
            novalidate>
            @csrf
            @method('PUT')
